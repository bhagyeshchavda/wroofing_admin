    ...
    <a href="{{ route('home') }}/admin" class="brand-link">
        @if (!empty($siteSidebarIcon))
            <span class="brand-text font-weight-light">
                <img src="{{ asset($siteSidebarIcon) }}" width="230px" alt="{{ $siteTitle }}" class="sidebar-img"
                    alt="sidebar-icon">
            </span>
        @else
    ...
    <div class="sidebar">
        ...
        <nav class="mt-2">
            <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu"
                ...
                @if (isset($usermenu))
                    @foreach ($usermenu as $key => $menu)
                        @if (in_array($key, $menus) || Auth::user()->role == 'admin')
                            ...
                            @endphp
                            <li
                                class="nav-item {{ isset($menu['sub']) && !empty($menu['sub']) ? (in_array(Request::path(), $menu['active_link']) || Request::is($active_link) ? 'has-treeview menu-open' : 'has-treeview') : '' }}">
                                <a href="{{ URL($menu['link']) }}"
                                    class="nav-link {{ in_array(Request::path(), $menu['active_link']) || Request::is($active_link) ? 'active' : '' }}">
                                        ...
                                        @foreach ($menu['sub'] as $submenu)
                                            ...
                                            <li class="nav-item">
                                                <a href="{{ URL($submenu['link']) }}"
                                                    class="nav-link {{ in_array(Request::path(), $submenu['active_link']) || Request::is($subactive_link) ? 'active' : '' }}">
                ...
                @endif
                <li class="nav-item ">
                    <a onclick="event.preventDefault(); document.getElementById('admin-logout-form').submit();"
                        data-turbolinks="false" href="{{ route('logout') }}" class="nav-link ">
